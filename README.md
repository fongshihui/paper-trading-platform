...
+------------------+         +----------------+         +-----------------+
|                  |<--(5)-- |                | <--(4)-- |                 |
|  Streamlit       |         |  Postgres      |         |  Kafka          |
|  Dashboard (Py)  |         |  Snapshots     |         |  ("signals")    |
|                  |         |                |         |                 |
+------------------+         +----------------+         +-----------------+
                                     ^
                                     |
...
4.  **Simulator**: A Python service consumes the `signals` topic. It simulates order execution by applying configurable slippage and transaction fees, maintains an internal portfolio state (positions, cash, P&L), and writes portfolio snapshots to Postgres.
5.  **Streamlit Dashboard**: A web-based dashboard reads the latest simulator snapshot from Postgres and displays live metrics, including an equity curve, open positions, P&L, drawdown, and volatility.
...
- `DATABASE_URL`: Postgres connection string for portfolio snapshots

### 2. Start the Containerized Platform
...
```sh
docker compose --profile alpaca up --build
```

Use only one producer profile at a time unless you intentionally want multiple market data sources publishing to the same topic.

### 3. Manual Flink Job Build and Submit

The Compose stack builds and submits the Flink job automatically through the `flink-job-submitter` service. If you need to submit it manually while debugging, compile the Java project and submit the resulting JAR to the running Flink cluster.
...
### 4. Run the Python Services Manually

The Compose stack runs these services for you. For local debugging outside Docker, open separate terminal sessions to run each component.
...
*   `DATABASE_URL`: Postgres connection string used by the simulator and dashboard.
...
- **Solution**: Check that `simulator/execution.py` is running and writing snapshots to Postgres
- **Check**: Verify `DATABASE_URL` and inspect the `portfolio_snapshots` table
...
*   **Durable State**: Extend the Postgres schema with separate tables for orders, fills, positions, and risk events.
